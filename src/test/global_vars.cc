...
{
  printk("The value of global_var %d is undefined\n", global_var);

  printk("The value of global_constant %d will always be the same\n", global_constant);

  printk("The function pointer to foo %p may be different on different processors\n", foo);
...
int go_global_vars(int argc, char **argv);
int go_global_vars(int argc, char **argv)
{
...
extern "C" void go_global_vars_c (int argc, char ** argv) {
    go_global_vars(argc, argv);
}
