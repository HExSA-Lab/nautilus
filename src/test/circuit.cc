...
union test_fl { unsigned ival; float fval; };
union test_dbl {unsigned long long uval; double dval; };
                    ...
                    Context ctx, HighLevelRuntime *runtime)
{
    ...
    printf("FAILURE!\n");
  {
    ...
    union test_dbl dt;
    dt.dval = sim_time;
    unsigned long long sim2 = (ts_end - ts_start);
    //printf("ELAPSED TIME = %7.3f s\n", sim_time);
    printf("ELAPSED TIME = 0x%016llx (double) s (%llu us) (start=%llu, end=%llu)\n", dt.uval, sim2, ts_start, ts_end);
    ...
    printf("GFLOPS = 0x%016llx (double) GFLOPS\n", dt.uval);
  }
			...
			int steps)
{
  ...
  int test = time(NULL);
  //printk("seeding random num generator with 0x%x\n", test);
  srand48(test);
