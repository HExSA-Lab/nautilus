                    ...
                    Context ctx, HighLevelRuntime *runtime)
{
  ...
  printk("Running hello world redux for %d points...\n", num_points);
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    {
      printk("Check failed for point %d: %d != %d\n", i, expected, received);
      all_passed = false;
    }
  }
  if (all_passed)
                     ...
                     Context ctx, HighLevelRuntime *runtime)
{
  ...
  printk("Hello world from task %d!\n", task->index_point.point_data[0]);
...
int go_index_task(int argc, char **argv);
int go_index_task(int argc, char **argv)
{
...
extern "C" void go_index_task_c (int argc, char ** argv) {
    go_index_task(argc, argv);
}
