                              ...
                              RegionAccessor<AccessorType::Generic, float> *fa_voltage)
{
  ...
  unsigned index = 0;
  {
    __m128 temp_v[WIRE_SEGMENTS+1] SSE_STACK_ALIGN;
    __m128 temp_i[WIRE_SEGMENTS] SSE_STACK_ALIGN;
    __m128 old_i[WIRE_SEGMENTS] SSE_STACK_ALIGN;
    __m128 old_v[WIRE_SEGMENTS-1] SSE_STACK_ALIGN;
    __m128 dt SSE_STACK_ALIGN;
    __m128 recip_dt SSE_STACK_ALIGN;
    dt = _mm_set1_ps(piece.dt);
    recip_dt = _mm_set1_ps(1.0/piece.dt);
    while ((index+3) < piece.num_wires)
    {
      ...
      temp_v[WIRE_SEGMENTS] = get_vec_node_voltage(current_wire, soa_pvt_voltage,
                                       soa_shr_voltage, soa_ghost_voltage,
                                       soa_in_ptr, soa_in_loc);
      __m128 inductance SSE_STACK_ALIGN;
      __m128 recip_resistance SSE_STACK_ALIGN;
      __m128 recip_capacitance SSE_STACK_ALIGN;
      inductance = _mm_load_ps(soa_inductance.ptr(current_wire));
      recip_resistance = _mm_rcp_ps(_mm_load_ps(soa_resistance.ptr(current_wire)));
      recip_capacitance = _mm_rcp_ps(_mm_load_ps(soa_wire_cap.ptr(current_wire)));
      for (int j = 0; j < steps; j++)
      {
        for (int i = 0; i < WIRE_SEGMENTS; i++)
        {
          __m128 dv SSE_STACK_ALIGN;
          __m128 di SSE_STACK_ALIGN;
          __m128 vol SSE_STACK_ALIGN;
          dv = _mm_sub_ps(temp_v[i+1],temp_v[i]);
          di = _mm_sub_ps(temp_i[i],old_i[i]);
          vol = _mm_sub_ps(dv,_mm_mul_ps(_mm_mul_ps(inductance,di),recip_dt));
          temp_i[i] = _mm_mul_ps(vol,recip_resistance);
        }
        for (int i = 0; i < (WIRE_SEGMENTS-1); i++)
        {
          __m128 dq SSE_STACK_ALIGN;
          dq = _mm_mul_ps(dt,_mm_sub_ps(temp_i[i],temp_i[i+1]));
