                    ...
                    Context ctx, HighLevelRuntime *runtime)
{
  ...
  for (GenericPointInRectIterator<1> pir(elem_rect); pir; pir++)
  {
    ...
    acc_x.write(DomainPoint::from_point<1>(pir.p), (double)rdtsc());
    //acc_y.write(DomainPoint::from_point<1>(pir.p), drand48());
    acc_y.write(DomainPoint::from_point<1>(pir.p), (double)rdtsc());
  }
  ...
  printk("Running daxpy computation with alpha %.8g...", alpha);
...
int go_phys_regs(int argc, char **argv);
int go_phys_regs(int argc, char **argv)
{
...
extern "C" void go_phys_regs_c(int argc, char ** argv) {
    go_phys_regs(argc, argv);
}
