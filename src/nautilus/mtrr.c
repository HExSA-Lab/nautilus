...
int nk_mtrr_get_features(int *num_var, int *flags)
{
    cpuid_ret_t c;
...
static uint32_t nk_mtrr_fixed_num_to_msr(int num)
{
    if (num<0) {
	goto out;
    }
...
static inline void gen_size_masks() 
{
    if (or_mask==-1) { 
	cpuid_ret_t c;
...
static inline int variable_match(void *ptr, int num, uint8_t *type)
{
...
static void mtrr_dump_xcall(void *arg)
{
	...
	nk_vc_printf_specific(c," smrr region => [%016lx, %016lx) type 0x%x %s %s\n",
