...
static prov_symtab* symtab;
static prov_sectab* sectab;

/* Binary Search over Section Table */
static prov_sectab_entry* sectab_lookup(uint64_t addr) {
	int start = 0, end = sectab->count;
	while(start <= end) {
		int middle = (start + end) / 2;
		prov_sectab_entry* entry = sectab->entries[middle];
		if((entry->start_addr <= addr) && (addr <= entry->end_addr))
			return entry;
		else if(addr < entry->start_addr)
			end = middle - 1;
...
/* Binary Search over Symbol Table */
static prov_symtab_entry* symtab_lookup(uint64_t addr) {
	int start = 0, end = symtab->count;
	while(start <= end) {
		int middle = (start + end) / 2;
		prov_symtab_entry* entry = symtab->entries[middle];
		if((entry->start_addr <= addr) && (addr <= entry->end_addr))
			return entry;
...
static void sectab_resolve_addr_ranges() {
	for(int i=0; i<(sectab->count); i++) {
		prov_sectab_entry* entry = sectab->entries[i];
...
static void symtab_resolve_addr_ranges() {
	for(int i=0; i<(symtab->count)-1; i++) {
		if(symtab->entries[i+1]->start_addr == -1)
			break;
...
static void sectab_init(uint64_t mod_start) {
	...
	sectab = (prov_sectab*) malloc(sizeof(prov_sectab));
	sectab->entries = (prov_sectab_entry**) malloc(sizeof(prov_sectab_entry*) * noe);
	sectab->count = noe;
	for(int i=0; i<noe; i++) {
		...
		sectab->entries[i] = (prov_sectab_entry*) malloc(sizeof(prov_sectab_entry));
...
static void symtab_init(uint64_t mod_start) {
	...
	symtab = (prov_symtab*) malloc(sizeof(prov_symtab));
	symtab->entries = (prov_symtab_entry**) malloc(sizeof(prov_symtab_entry*) * noe);
	symtab->count = noe;
	for(int i=0; i<noe; i++) {
		symentry* entry = (symentry*) (((char*)(syms_base_addr)) + offset + (sizeof(symentry)*i));
		char* symbol = (char*) ((char*)(syms_base_addr+3) + entry->offset);
		uint64_t addr = entry->value;
		symtab->entries[i] = (prov_symtab_entry*) malloc(sizeof(prov_symtab_entry));
		symtab->entries[i]->symbol = symbol;
	...
	for(int i=0; i<sectab->count; i++) {
		prov_sectab_entry* entry = sectab->entries[i];
		PROV_DEBUG("[%08d]\t%08x\t%08x\t%s\n", i+1, entry->start_addr, entry->end_addr, entry->symbol);
	}
}

static char* prov_get_section(uint64_t addr) {
	prov_sectab_entry* entry = sectab_lookup(addr);
	return (entry != NULL) ? entry->name : NULL;
}

static char* prov_get_symbol(uint64_t addr) {
	prov_symtab_entry* entry = symtab_lookup(addr);
	return (entry != NULL) ? entry->symbol : NULL;
}

provenance_info prov_get_info(uint64_t addr) {
	provenance_info prov_info;
	prov_info.symbol = prov_get_symbol(addr);
	prov_info.section = prov_get_section(addr);
